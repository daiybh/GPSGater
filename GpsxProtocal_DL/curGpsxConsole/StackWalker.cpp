...
public:
  ...
  BOOL Init(LPCSTR szSymPath)
  {
    ...
    {
      _tcscat_s(szTemp, _countof(szTemp),_T(".local"));
      if (GetFileAttributes(szTemp) == INVALID_FILE_ATTRIBUTES)
      {
        // ".local" file does not exist, so we can try to load the dbghelp.dll from the "Debugging Tools for Windows"
        if (GetEnvironmentVariable(_T("ProgramFiles"), szTemp, 4096) > 0)
        {
          _tcscat_s(szTemp, _countof(szTemp),_T("\\Debugging Tools for Windows\\dbghelp.dll"));
        ...
        {
          _tcscat_s(szTemp, _countof(szTemp),_T("\\Debugging Tools for Windows 64-Bit\\dbghelp.dll"));
          if (GetFileAttributes(szTemp) != INVALID_FILE_ATTRIBUTES)
          {
            m_hDbhHelp = LoadLibrary(szTemp);
          }
...
BOOL StackWalker::ShowCallstack(HANDLE hThread, const CONTEXT *context, PReadProcessMemoryRoutine readMemoryFunction, LPVOID pUserData)
{
  ...
  p_info = (PEXCEPTION_POINTERS)pUserData;
...
void StackWalker::OnSymInit(LPCSTR szSearchPath, DWORD symOptions, LPCSTR szUserName)
{
	...
	//CString strTime = time1.Format(_T("::%Y-%m-%d %H:%M:%S"));
	//USES_CONVERSION;
	//char* pszChar = W2A(strTime);
...
void StackWalker::generate_report(PEXCEPTION_POINTERS p_info)
{
	...
	_snprintf_s(buffer, STACKWALK_MAX_NAMELEN, "Unhandled exception: %02d/%02d/%04d %02d:%02d:%02d\r\n", st.wMonth, st.wDay, st.wYear, st.wHour, st.wMinute, st.wSecond);
	OnOutput(buffer);	

	_snprintf_s(buffer, STACKWALK_MAX_NAMELEN, "Thread Information: ProgressID[0x%x] ThreadID[0x%x]\n",GetCurrentProcessId(),GetCurrentThreadId() );
	OnOutput(buffer);	

	PEXCEPTION_RECORD p_record = p_info->ExceptionRecord;
	
	// First print information about the type of fault
	_snprintf_s(buffer, STACKWALK_MAX_NAMELEN, "Exception code: %08X %s\r\n", p_record->ExceptionCode, get_exception_string(p_record->ExceptionCode));
	OnOutput(buffer);	
...
// 	write(h_file, "Fault address:  %08X %02X:%08X %s\r\n", p_record->ExceptionAddress, section, offset, sz_module);
	...
	OnOutput(buffer);	
	_snprintf_s(buffer, STACKWALK_MAX_NAMELEN, "EAX:%08X\r\nEBX:%08X\r\nECX:%08X\r\nEDX:%08X\r\nESI:%08X\r\nEDI:%08X\r\n", p_context->Eax, p_context->Ebx, p_context->Ecx, p_context->Edx, p_context->Esi, p_context->Edi );
	OnOutput(buffer);	
	_snprintf_s(buffer, STACKWALK_MAX_NAMELEN, "CS:EIP:%04X:%08X\r\n", p_context->SegCs, p_context->Eip);
	OnOutput(buffer);	
	_snprintf_s(buffer, STACKWALK_MAX_NAMELEN, "SS:ESP:%04X:%08X  EBP:%08X\r\n", p_context->SegSs, p_context->Esp, p_context->Ebp );
	OnOutput(buffer);	
	_snprintf_s(buffer, STACKWALK_MAX_NAMELEN, "DS:%04X  ES:%04X  FS:%04X  GS:%04X\r\n", p_context->SegDs, p_context->SegEs, p_context->SegFs, p_context->SegGs );
	OnOutput(buffer);	
	_snprintf_s(buffer, STACKWALK_MAX_NAMELEN, "Flags:%08X\r\n", p_context->EFlags);
	OnOutput(buffer);	
#endif //_M_IX86	
	_snprintf_s(buffer, STACKWALK_MAX_NAMELEN, "\r\n");
	OnOutput(buffer);	
