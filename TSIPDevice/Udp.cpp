...
BOOL Udp::Open(string bindIp, int bindPort, string multicastIp)
{
	if (m_isOpen)
		return FALSE;
	...
	// 绑定套接字
	memset((PVOID)&m_BindAddr, 0, sizeof(SOCKADDR_IN));
	...
	// 设置采集地址
	if ( ! SetMulticast( multicastIp.c_str()) )
		return FALSE;

	m_isOpen = TRUE;
	return TRUE;
}
int Udp::Write(const char *connectIp,int connectPort,PBYTE pBuffer, UINT16 bufferSize, UINT nTimeOut /*= 500000*/)
{
	if(!m_isOpen)
		return FALSE;
	...
	m_SendAddr.sin_port = htons(connectPort);
	m_SendAddr.sin_addr.s_addr = inet_addr(connectIp);
...
BOOL Udp::Connect(string connectIp, int connectPort)
{
	...
	m_SendAddr.sin_family = AF_INET;
	m_SendAddr.sin_port = htons(connectPort);
	m_SendAddr.sin_addr.s_addr = inet_addr(connectIp.c_str());

	//if (connect(m_Socket, (SOCKADDR*)&m_SendAddr, sizeof(m_SendAddr)) == SOCKET_ERROR )
	//{
	//	ReportError();
	//	return FALSE;
	//}
...
int Udp::Read_UDP( BYTE* pBuffer, int bufferSize,char **pFromAddr,int &nFromPort, UINT nTimeOut/*=500000*/ )
{
	int iRead;
	...
	iRead = Socket::Read_UDP(pBuffer, bufferSize,pFromAddr,nFromPort, nTimeOut);
...
BOOL Udp::SetMulticast( PCSTR ip_addr )
{
	int error = 0;
	int i_val = 0;

	// 设置多播和广播
	if ( IN_MULTICAST(ntohl(inet_addr(ip_addr))) )
	{
		i_val = 1;
		...
		multicastAddr.imr_multiaddr.s_addr = inet_addr(ip_addr);
		multicastAddr.imr_interface.s_addr = m_BindAddr.sin_addr.s_addr;
	...
	//设置广播
	if ( inet_addr(ip_addr) == INADDR_BROADCAST )
	{
		i_val = 1;
