...
BOOL I_XMLParser::CreateInstance(I_XMLParser **ppIXMLParser)
{
	if(ppIXMLParser==NULL)
		return FALSE;
	*ppIXMLParser = (I_XMLParser *)new CXMLParser2();
...
BOOL CXMLParser3::Parse(CString strXML)
{
	try
	{
		BSTR bstr = strXML.AllocSysString();
		if(m_pXMLDom->loadXML(bstr) == VARIANT_TRUE)
		{
...
CString CXMLParser3::GetRootName()
{
	return (LPCTSTR)m_pXMLDom->documentElement->nodeName;
}
...
CString CXMLParser3::GetNodeText(CString strNodeName)
{
	try
	{
		HRESULT hr;
...
CString CXMLParser3::GetNodeText(int nIndex)
{
	if(nIndex >= m_pNodeList->length)
		return _T("");
	return (LPCWSTR)m_pNodeList->item[nIndex]->text;
}
...
BOOL CXMLParser3::GetRoot(CString &strName, CString &strPara)
{
	try
	{
		MSXML2::IXMLDOMElementPtr pRoot = m_pXMLDom->documentElement;
...
BOOL CXMLParser3::GetNodesText(CString strNodeName, std::vector<CString>& vText)
{
	...
	try
	{
		...
		{
			MSXML2::IXMLDOMNodePtr Node = List->item[i];
			vText.push_back((BSTR)Node->text);
		}
		return TRUE;
	}
	catch (_com_error &e)
